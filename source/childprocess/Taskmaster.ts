	...
	export class Taskmaster {
		...
		processPullrequest() {
			...
			this.repositoryConfiguration.dependencies.forEach(repo => {
				...
				var feature = this.upstreamBranch.split(/[^A-Za-z]/)[0];
				var featurebranch = this.branches[feature] + this.upstreamBranch.split(feature)[1];
				var thirdCloneFallbackJob = new Job("Dependency third fallback Git clone upstream", "git", ["clone", "-b", "develop", "--single-branch", secondaryOrganizationUrl],this.folderPath, true);						
                        	var secondCloneFallbackJob = new Job("Dependency second fallback Git clone upstream", "git", ["clone", "-b", this.upstreamBranch, "--single-branch", secondaryOrganizationUrl], this.folderPath, true, thirdCloneFallbackJob);
                       		var firstCloneFallbackJob = new Job("Dependency fallback Git clone upstream", "git", ["clone", "-b", featurebranch, "--single-branch", secondaryOrganizationUrl], this.folderPath, true, secondCloneFallbackJob);
				...
				var firstPullFallbackJob = new Job("Dependency fallback Git pull upstream", "git", ["pull", secondaryOrganizationUrl, featurebranch], secondaryRepositoryFolderPath, true, secondPullFallbackJob);
